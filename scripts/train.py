...
def main():
    ...
    # Initialize wandb
    wandb_run_name = args.run_name if args.run_name else f"{args.dataset}_{mode.lower()}"
    wandb.init(
        project="diffuser-training",
        name=wandb_run_name,
        config=vars(args),
    ...
    trainer = Trainer(
        ...
        wandb_run=wandb.run,  # Pass wandb run to trainer
    )
